...
#define DBName @"EASYSQL.sqlite"
#define DBParentPrefix @"EASYSQLParentID_"
#define DBChildPrefix  @"EASYSQLChildID_"
#define kPId  @"__pid__"
#define kCId  @"__cid__"

#ifdef DEBUG
#ifdef EASYSQLBUG
#define EASYSQLLog(fmt, ...) NSLog((@"%s [Line %d]\n" fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
#else
#define EASYSQLLog(...)
#endif
#else
...
@interface NSDate (EASYSQLHandle)
...
@implementation NSDate (EASYSQLHandle)
...
@interface NSString (EASYSQLHandle)
...
@interface NSObject (EASYSQLHandle)
...
@implementation NSObject (EASYSQLHandle)
...
{
    @synchronized(self){
        static EASYSQLHandle *EASYSQL;
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            EASYSQL = [[EASYSQLHandle alloc] init];
#ifdef EASYSQL_EncryptEnable
            EASYSQL.encryptEnable = YES;
#endif
        });
        return EASYSQL;
    }
    ...
    @synchronized(self){
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:dbPath]) {
            ...
            if (extDbPath) {
                ...
                if (rc) {
                    NSArray *tables = [EASYSQLHandle sqlite_tablename];
                    for (NSString *table in tables) {
                        NSMutableString *sql;
                        
                        sqlite3_stmt *stmt = NULL;
                        NSString *str = [NSString stringWithFormat:@"select sql from sqlite_master where type='table' and tbl_name='%@'", table];
                        EASYSQLHandle *EASYSQL = [EASYSQLHandle shareDb];
                        [EASYSQLHandle openDb];
                        if (sqlite3_prepare_v2(EASYSQL->_sqlite3DB, [str UTF8String], -1, &stmt, NULL) == SQLITE_OK) {
                        ...
                        int ret = sqlite3_exec(EASYSQL.sqlite3DB, [createTempDb UTF8String], NULL, NULL, &errmsg);
                        if (ret != SQLITE_OK) {
                            NSLog(@"%s", errmsg);
                        }
                        ...
                        ret = sqlite3_exec(EASYSQL.sqlite3DB, [importDb UTF8String], NULL, NULL, &errmsg);
                        if (ret != SQLITE_OK) {
                            NSLog(@"%s", errmsg);
                        }
                        // 删除旧表
                        NSString *dropDb = [NSString stringWithFormat:@"drop table %@", table];
                        ret = sqlite3_exec(EASYSQL.sqlite3DB, [dropDb UTF8String], NULL, NULL, &errmsg);
                        if (ret != SQLITE_OK) {
                            NSLog(@"%s", errmsg);
                        }
                        // 创建新表
                        ...
                        ret = sqlite3_exec(EASYSQL.sqlite3DB, [createDb UTF8String], NULL, NULL, &errmsg);
                        if (ret != SQLITE_OK) {
                            NSLog(@"%s", errmsg);
                        }
                        ...
                        if ([[self shareDb] encryptEnable]) {
                            ...
                            for (EASYSQLObject *obj in results) {
                                
                                [self openDb];
                                
                                NSMutableArray *keys = [NSMutableArray arrayWithCapacity:0];
                                [EASYSQLHandle class:obj.class getPropertyKeyList:keys];
                                
                                NSMutableArray *types = [NSMutableArray arrayWithCapacity:0];
                                [EASYSQLHandle class:obj.class getPropertyTypeList:types];
...
+ (BOOL)openDb
{
    @synchronized(self){
        NSString *dbPath = [EASYSQLHandle dbPath];
        EASYSQLHandle *db = [EASYSQLHandle shareDb];
    ...
    //        EASYSQLLog(@"数据库已打开");
        ...
        if (rc == SQLITE_OK) {
    //        EASYSQLLog(@"打开数据库%@成功!", dbPath);
            
            db.isOpened = YES;
            return YES;
        } else {
            EASYSQLLog(@"打开数据库%@失败!", dbPath);
            return NO;
        }
...
+ (BOOL)closeDb {
    @synchronized(self){
    #ifdef EASYSQLBUG
        NSString *dbPath = [EASYSQL dbPath];
    #endif
    ...
    //        EASYSQLLog(@"数据库已关闭");
        ...
        if (rc == SQLITE_OK) {
    //        EASYSQLLog(@"关闭数据库%@成功!", dbPath);
            db.isOpened = NO;
            db.sqlite3DB = NULL;
            return YES;
        } else {
            EASYSQLLog(@"关闭数据库%@失败!", dbPath);
            return NO;
        }
        return YES;
    }
...
+ (void)createDbTable:(Class)aClass
{
    @synchronized(self){
        ...
        if ([EASYSQLHandle sqlite_tableExist:aClass]) {
            EASYSQLLog(@"数据库表%@已存在!", NSStringFromClass(aClass));
            return;
        }
        ...
        [EASYSQLHandle class:aClass getPropertyNameList:propertyArr primaryKeys:primaryKeys];
 ...
 *	@param 	obj 	数据对象
 */
- (BOOL)insertDbObject:(EASYSQLObject *)obj;
{
    @synchronized(self){
 ...
 *	@param 	obj 	数据对象
 */
- (BOOL)replaceDbObject:(EASYSQLObject *)obj;
{
    @synchronized(self){
 ...
 *	@param 	obj 	数据对象
 */
- (BOOL)insertDbObject:(EASYSQLObject *)obj forced:(BOOL)forced
{
    @synchronized(self){
        if (![EASYSQLHandle isOpened]) {
            [EASYSQLHandle openDb];
        ...
        if (![EASYSQLHandle sqlite_tableExist:obj.class]) {
            [EASYSQLHandle createDbTable:obj.class];
        }
        ...
        NSMutableArray *propertyArr = [NSMutableArray arrayWithCapacity:0];
        propertyArr = [NSMutableArray arrayWithArray:[EASYSQLHandle sqlite_columns:obj.class]];
    ...
    //    NSMutableArray *subDbObjects = [NSMutableArray arrayWithCapacity:0];
        ...
        if (sqlite3_prepare_v2(sqlite3DB, [sql_NSString UTF8String], -1, &stmt, &errmsg) == SQLITE_OK) {
            for (int i = 1; i <= argNum; i++) {
                ...
                id value;
                NSInteger rowId = [EASYSQLHandle lastRowIdWithClass:obj.class];
                ...
                if ([key hasPrefix:DBParentPrefix]) {
                    ...
                    if ([object isKindOfClass:[EASYSQLObject class]]) {
                        NSInteger subDbRowId = [EASYSQLHandle lastRowIdWithClass:object.class];
                        value = [[NSString alloc] initWithFormat:@"%@", @(subDbRowId+1)];
                        
                        EASYSQLObject *dbObj = (EASYSQLObject *)object;
                ...
                if ([column_type_string isEqualToString:@"blob"]) {
                    ...
                    if (!value || value == [NSNull null] || [value isEqual:@""]) {
                        ...
                        value = [EASYSQLHandle valueForDbObjc_property_t:property_t dbValue:value _id:rowId];
...
//    @synchronized(self){
        ...
        // 清除过期数据
        [EASYSQLHandle cleanExpireDbObject:aClass];
        ...
        if (sqlite3_prepare_v2(sqlite3DB, [selectstring UTF8String], -1, &stmt, NULL) == SQLITE_OK) {
            ...
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                ...
                EASYSQLObject *obj = [[NSClassFromString(tableName) alloc] init];
                ...
                for (int i = 0; i < column_count; i++) {
                    ...
                    if ([obj_column_decltype isEqualToString:@"text"]) {
                        ...
                        if (value != NULL) {
                            ...
                            id objValue = [EASYSQLHandle valueForObjc_property_t:property_t dbValue:column_value];
                            if (objValue) {
                        ...
                        if (databyte != NULL) {
                            ...
                            id objValue = [EASYSQLHandle valueForObjc_property_t:property_t dbValue:column_data];
                            [obj setValue:objValue forKey:key];
                        }
                    } else {
                        const unsigned char *value = sqlite3_column_text(stmt, i);
...
- (BOOL)removeDbObjects:(Class)aClass condition:(NSString *)condition
{
    @synchronized(self){
        ...
        sqlite3 *sqlite3DB = [[EASYSQLHandle shareDb] sqlite3DB];
        
        NSString *tableName = NSStringFromClass(aClass);
        
        // 删掉表
        if (!condition || [[condition lowercaseString] isEqualToString:@"all"]) {
            return [EASYSQLHandle removeDbTable:aClass];
        }
 ...
 *	@return	修改是否成功
 */
- (BOOL)updateDbObject:(EASYSQLObject *)obj condition:(NSString *)condition
{
//    @synchronized(self){
        if (![EASYSQLHandle isOpened]) {
            [EASYSQLHandle openDb];
        ...
        NSMutableArray *propertyTypeArr = [NSMutableArray arrayWithArray:[EASYSQLHandle sqlite_columns:obj.class]];
        ...
        for (int i = 0; i < count; i++) {
            ...
            id value = [EASYSQLHandle valueForDbObjc_property_t:property dbValue:objValue _id:-1];
            ...
            for (NSString *key in keys) {
                ...
                if ([column_type_string isEqualToString:@"blob"]) {
                    ...
                    if (!value || value == [NSNull null] || [value isEqual:@""]) {
                        ...
                        value = [EASYSQLHandle valueForDbObjc_property_t:property_t dbValue:value _id:obj.__id__];
                        NSString *column_value = [NSString stringWithFormat:@"%@", value];
...
+ (BOOL)cleanExpireDbObject:(Class)aClass
{
    @synchronized(self){
        ...
        [[[EASYSQLHandle alloc] init] removeDbObjects:aClass condition:condition];
    ...
    @synchronized(self){
        ...
        switch(type[0]) {
                ...
                //Handle different clases in here
            {
                ...
                if ([NSClassFromString(cls) isSubclassOfClass:[EASYSQLObject class]]) {
                    return DBText;
                }
            }
                break;
                    ...
                    for (NSString *key in dict) {
                        ...
                        if ([obj isKindOfClass:[NSString class]]) {
                            ...
                            if ([dbObj hasPrefix:DBChildPrefix]) {
                                ...
                                EASYSQLObject *child = (EASYSQLObject *)[NSClassFromString(clsName) dbObjectsWhere:where orderby:nil][0];
                ...
                //Handle different clases in here
            {
                ...
                if ([NSClassFromString(cls) isSubclassOfClass:[NSDictionary class]]) {
                    ...
                    for (NSString *key in dbValue) {
                        ...
                        if ([obj isKindOfClass:[EASYSQLObject class]]) {
                            EASYSQLObject *dbObject = (EASYSQLObject *)obj;
...
+ (void)class:(Class)aClass getPropertyNameList:(NSMutableArray *)proName primaryKeys:(NSMutableArray *)primaryKeys
{
        ...
        for (int i = 0; i < count; i++) {
            objc_property_t property = properties[i];
            NSString * key = [EASYSQLHandle dbNameConvertFromObjc_property_t:property];
            NSString *type = [EASYSQLHandle dbTypeConvertFromObjc_property_t:property];
        ...
        if (aClass == [EASYSQLObject class]) {
            return;
        }
        [EASYSQLHandle class:[aClass superclass] getPropertyNameList:proName primaryKeys:primaryKeys];
...
+ (void)class:(Class)aClass getPropertyKeyList:(NSMutableArray *)proName
{
    @synchronized(self){
        ...
        [EASYSQLHandle class:[aClass superclass] getPropertyKeyList:proName];
    }
}
...
+ (void)class:(Class)aClass getPropertyTypeList:(NSMutableArray *)proName
{
    @synchronized(self){
        ...
        [EASYSQLHandle class:[aClass superclass] getPropertyTypeList:proName];
    }
}
...
@interface NSData (EASYSQLHandle)
...
@implementation NSData (EASYSQLHandle)
...
@implementation NSString (EASYSQLHandle)
...
objc_property_t * st_class_copyPropertyList(Class cls, unsigned int *count)
{
    if (![cls isSubclassOfClass:[EASYSQLObject class]]) {
        return NULL;
    };
